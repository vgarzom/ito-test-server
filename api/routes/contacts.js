...
router.post('/login', function (req, res, next) {
    ...
    Contact.find({ username: username, password: pass }, function (err, contacts) {
        if (err) return res.status(200).send({ code: 500, message: err });
        if (contacts.length > 0) {
            res.status(200).send({ code: 200, message: "Bienvenido", contact: contacts[0] })
        }
        else res.status(200).send({ code: 201, message: "Usuario o contraseña incorrecto" })
    });
});
